...
BrailleSCARA is a 4-DOF SCARA robot that converts printed text into Braille documents and draws tactile diagrams --- enabling blind students to read textbooks and touch STEM illustrations for the first time, at a system cost under \textcurrency 1 Lakh.

\textbf{How it works --- plain and simple:}
\begin{enumerate}[noitemsep]
    \item \textbf{Dual Input Mode:} 
    \begin{itemize}[noitemsep, label={-}]
        \item \textit{Physical:} A camera reads a printed page. ADI's MAX78000 runs OCR on-chip to extract text.
        \item \textit{Digital:} Teachers can directly upload PDF documents or SVG diagrams via Bluetooth using the MAX32690 BLE 5.2 microcontroller.
    \end{itemize}
    \item Software converts the recognized text to Bharati Braille encoding and generates dot coordinates following ISO 17049 spacing standards.
    \item The SCARA arm picks a blank Braille paper sheet from a tray and places it on the embossing bed.
    \item The arm moves to each dot position with $\pm$0.05 mm accuracy using ADI's ADSP-CM419 + TMC4671 closed-loop FOC drive. A solenoid fires for 50 ms, pushing a 2 mm steel pin through the paper into an anvil hole, creating a raised dot.
    \item After each dot, a force sensor read by ADI's AD7124-8 (24-bit ADC) verifies the dot formed correctly. If the dot is too shallow, the arm re-embosses it immediately.
    \item For tactile diagrams (maps, graphs, geometric figures), the arm traces freeform paths with a spring-loaded ball stylus, creating raised lines that blind students can feel with their fingertips.
    \item Completed pages are picked and sorted into output trays by page number.
\end{enumerate}
...
India has \textbf{15 million blind people} --- more than any country on Earth. Only \textbf{1\% can read Braille.} Not because they can't learn. Because the materials don't exist. 
There are \textbf{25 Braille presses} for 1.4 billion people. A single Braille textbook costs 10--50$\times$ its printed version. 

But the harder, completely unsolved problem is \textbf{tactile graphics.} Education is not just text --- it is diagrams. The human heart. India's map. A parabola. A circuit schematic. A triangle construction. Sighted students see these on every page. \textbf{Blind students get nothing.}

Existing Braille machines (BrailleRAP, Index Braille) produce text only --- rows of dots. They cannot draw freeform diagrams. The machines that CAN draw tactile graphics (ViewPlus Tiger, PIAF) cost \textbf{\textcurrency 3--25 Lakh} and are found only in a handful of elite institutions. \textbf{There is no device under \textcurrency 10 Lakh in India that can produce a touchable version of a science textbook diagram.}
...
\vspace{1em}
\noindent\framebox[\textwidth]{\parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}{\centering\vspace{1cm}\textbf{[TODO: INSERT FORCE ANALYSIS GRAPH HERE]}\\Show calculated solenoid force vs. paper deformation to prove it won't tear.\vspace{1cm}}}
\vspace{1em}
...
\textbf{Vision spot-check (per Braille cell):} After completing a 6-dot cell, the MAX78000 camera captures an image. A lightweight CNN (3 conv + 2 FC layers, $<$200K weights, 8-bit quantized) classifies PASS or FAIL. \textit{Dataset Strategy: Since no public dataset exists for embossed Braille quality, we will generate a custom dataset in Phase 3 by having the arm emboss 5,000+ test dots and labeling them via the camera feed. The OCR model will be fine-tuned on the public EMNIST dataset for printed text.}
...
\begin{figure}[H]
    ...
    \caption{System Architecture showing integration of 12 ADI Products}
\end{figure}

\textbf{ADI Products Used (12 --- each genuinely needed):}
\begin{table}[H]
\centering
\resizebox{\textwidth}{!}{%
\begin{tabular}{lll}
\toprule
\textbf{\#} & \textbf{Product} & \textbf{Why It's Needed} \\
\midrule
1 & ADSP-CM419 & Motor control DSP --- runs position/velocity PID at 10 kHz \\
2 & TMC4671 $\times$4 & Hardware FOC --- sinusoidal commutation per joint \\
...
6 & AD8418A $\times$4 & Current sense --- FOC phase current feedback \\
7 & ADuM4135 $\times$4 & Isolated gate drivers --- MOSFET H-bridge driving \\
...
\vspace{1em}
\noindent\framebox[\textwidth]{\parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule\relax}{\centering\vspace{2cm}\textbf{[TODO: INSERT GAZEBO SIMULATION SCREENSHOT HERE]}\\Replace this box with an image of your simulated SCARA arm to prove you have already started.\vspace{2cm}}}
\vspace{1em}
